...
const BtnEncontrarUsuarios: React.FC<BtnEncontrarUsuariosProps> = ({
  ...
  const getDefaultText = () => {
    ...
    return "Encontrar usuários";
  };

  const baseClasses =
    "inline-block font-semibold rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2";
  ...
  const finalClasses =
    `${baseClasses} ${variantClassBtn[variant]} ${sizeClasses[size]} ${disabledClasses} ${className}`.trim();
